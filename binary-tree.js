...
class BinaryTree {
  ...
  minDepth() {
    ...
    function minDepthHelper(node) {
      ...
      // if left or right is null, traverse the opposite child
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length) {
      ...
      let currentVal = currNode.val;
      // check that current val is larger than lower bound
      let higherThanLowerBound = currentVal > lowerBound;
      // check if current value is less than current closest or closest is null
      let shouldReassignClosest = currentVal < closest || closest === null;

      // reassign closest to current value if both conditions are true
      if (higherThanLowerBound && shouldReassignClosest) {
        closest = currentVal;
      }

      // push children in queue if they exist
